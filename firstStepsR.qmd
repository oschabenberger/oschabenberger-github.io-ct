...
account. These are available for free [here](https://posit.cloud/plans/free).
:::
...
Python, and other code within the same document. 
...
-   **Help**. This pane contains many useful tabs, such as a File browse, package 
...
The `R` installation comes with attached base packages, you do not need to install 
or load those. To access capabilities in other packages you need to go through
a two-step process:
...
needs to use the functionality of the package.

A **standard** `R` package is made available through the [CRAN](https://cran.r-project.org/) 
(Comprehensive R Archive Network) repositories. For the analytic work in this module
we need to install three packages from CRAN: `ISLR2`, `rpart` and `rpart.plot`.

Copy the following line of code to the console in `RStudio` and hit <Return>.
...
- `cp`: Threshold for the complexity parameter. The complexity of the tree is 
measured during pruning as
$$
